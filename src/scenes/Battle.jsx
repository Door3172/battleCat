...
export default function Battle({ coins, setCoins, currentStage, setScene, highestUnlocked, setHighestUnlocked, lineup, unlocks, addEnemyName }){
  ...
  const upgradeIncome=()=>{
    ...
    w.fish-=w.incomeCost; w.income+=3.2; w.incomeLv+=1; w.incomeCost=Math.round(w.incomeCost*1.6);
  ...
  const BattleControls=()=>{
    ...
    return (
      <>
        <SlotTray>
          {lineup.map((k,i)=>{ const cd=w.summonCd[k]||0; const disabled=cd>0||ui.state!=='running'; return (
