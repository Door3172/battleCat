...
export function spawnEnemy(world, getCanvasWidth, getCanvasHeight, onEnemySeen){
  ...
  if(cfg.isBoss && !world.bossSpawned && (world.rightHp<=600 || world.time>=40)){
    ...
    const scaled = {
      ...base,
      hp:Math.round(base.hp*cfg.difficulty),
      maxHp:Math.round(base.hp*cfg.difficulty),
      attack:Math.round(base.attack*(1.0+(cfg.difficulty-1)*0.6))
    };
  ...
  const tpl = {
    ...
    hp:Math.round(base.hp*cfg.difficulty),
    maxHp:Math.round(base.hp*cfg.difficulty),
    attack:Math.round(base.attack*(0.9+Math.random()*0.2)*cfg.difficulty)
  };
  const cw = getCanvasWidth();
      ...
      if(inRange){
        if(u.aoe){
          ...
          for(let k=0;k<world.units.length;k++){
            ...
            const d = Math.abs(t.x - target.x); // 橫向 1D 距離（你的遊戲是橫版）
            if(d <= r){
              t.hp -= u.atk;
              hits++;
